...
const MyList = () => {
  ...
  useEffect(() => {
    fetch(
      `https://b9a10-tourism-management-server-mu.vercel.app/myList/${user?.email}`
    )
      .then((res) => res.json())
      .then((data) => {
        setItem(data);
  ...
  const handleDelete = (_id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        fetch(
          `https://b9a10-tourism-management-server-mu.vercel.app/spot/${_id}`,
          {
            method: "DELETE",
            headers: {
            ...
            {item?.map((p, idx) => (
              <tr key={idx}>
                ...
                <td className="flex enter gap-2 items-center pt-9">
                  ...
                  <Link
                    to={`/update/${p._id}`}
                    className="btn bg-green-600 border-0 text-white font-semibold text-lg"
                  >
                    Update
                  </Link>
                </td>
